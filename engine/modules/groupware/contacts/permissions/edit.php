...
if (ACTION == "change") {
	$permission = prepareForView($connection->execute('select * from contact_permissions_list where contact_id = '.CONTACTLIST_ID.' and account_id = '.PERMISSION_ID)->fetch());
} else {
	$permission = array('permission_id' => Constants::PERMISSION_READ_ONLY);
}
?>
<?php require_once(COMMON_MODULES_PATH.DIRECTORY_SEPARATOR."public.php"); ?>
<form width="100%" height="100%" align="right" action="/?module=<?= getParentModule(); ?>/<?= getChildModule(); ?>/list&action=<?= ACTION; ?><?= (defined("CONTACTLIST_ID")?"&contactlist_id=".CONTACTLIST_ID:"").(defined("PERMISSION_ID")?"&permission_id=".PERMISSION_ID:""); ?>" method="POST">
