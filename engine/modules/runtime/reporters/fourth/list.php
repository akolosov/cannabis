...
function printTitle() {
	...
	print "<h2 onClick=\"window.print();\">Расширенный отчет о ходе выполнения Служебных записок</h2>\n";
...
function printSplitter() {
	...
	print "<th class=\"groupsplitter\" colspan=\"10\"></th>\n";
	print "</tr>\n";
...
function printHeader() {
	...
	print "<tr>\n";
	print "<th class=\"report\" width=\"30%\">Документ №</th>\n";
...
function printGroupHeader() {
	...
	switch ($report_params['by_group']) {
		case 'project':
			...
			print "<td class=\"report group big_italic\" colspan=\"9\">".$project_name."</td></tr>\n";
			break;

		case 'initiator':
			print "<tr><td class=\"report group\" align=\"right\">Разработчик:</td>\n";
			print "<td class=\"report group big_italic\" colspan=\"9\">".$initiator_name."</td></tr>\n";
			break;

		case 'status':
			print "<tr><td class=\"report group\" align=\"right\">Статус:</td>\n";
			print "<td class=\"report group big_italic\" colspan=\"9\">".$status_name."</td></tr>\n";
			break;
...
function printBody($process, $group_by) {
	...
	if ($print_this_line) {
		...
		print "</a></td>\n";
		print "<td class=\"report center\" rowspan=\"7\">";
		...
		print "<td class=\"report center\" rowspan=\"7\">".getPropertyValueByName($properties, 'Срок исполнения').(getPropertyValueByName($properties, 'Срок исполнения')?" ".END_WORK_TIME:"")."</td>\n";
		print "<td class=\"report center\" rowspan=\"7\">";
		if ($process['version'] == 1) {
		...
		print "</td>\n";
		print "<td class=\"report center\" rowspan=\"7\">".getPropertyValueByName($properties, 'Дата и время выполнения Исполнителем')."</td>\n";
		...
		print "<td rowspan=\"7\" class=\"report center";
...
function printFooter() {
	...
	print "<tr>\n";
	print "<td rowspan=\"2\" colspan=\"3\" class=\"group report\" align=\"right\">ИТОГО ПО ВСЕМ ГРУППАМ:</td>\n";
...
function printGroupFooter($group_by) {
	...
	print "<td rowspan=\"2\" colspan=\"3\" class=\"group report\" align=\"right\">ИТОГО ПО ГРУППЕ:</td>\n";
				...
				foreach ($projectslist as $project_id => $project_name) {
					...
					if (isNotNULL($processes)) {
						...
						foreach ($processes as $process) {
							...
							if (((!$first_page) and ($total_on_page == 10)) or (($first_page) and ($total_on_page == 23))) {
								printSplitter();
							}
						}
