...
<?php
	...
	if ((is_null($project)) or ($project->getProperty('id') <> PROJECT_INSTANCE_ID)) {
		...
		logRuntime('[Managers.Processes->List] reinitialize process instance by id '.PROCESS_INSTANCE_ID);
		$process = $project->reinitProcess(PROCESS_INSTANCE_ID);
	}

	if ((ACTION == 'undo2action') and (defined('ACTION_ID'))) {
		$process->undoToAction(ACTION_ID);
	}
...
<table width="100%" align="center">
	...
	<?php
	print "<div class=\"project\"><b>Предприятие: </b>".$project->getProperty('name').' (статус: '.$project->getProperty('statusname').")</div>";
	print "<div class=\"process\"><b>Документ: </b>".$process->getProperty('name').' №'.$process->getProperty('id').' (статус: '.$process->getProperty('statusname').(($process->haveChilds() && $process->haveIncomletedChilds())?' [ожидает завершения работы дочерних документов]':'').")</div>\n";
	...
	foreach ($actions as $action) {
		...
		if (($user_permissions[getParentModule()][getParentChildModule()]['can_admin']) and ($action->getProperty('type_id') <> Constants::ACTION_TYPE_INFO)) {
			print "&nbsp;<img class=\"action\" src=\"images/edit.png\" onClick=\"hideIt('action_edit_".$action->getProperty('id')."')\" title=\"Изменение значения действия\" /><div class=\"prop_info\" id=\"action_edit_".$action->getProperty('id')."\"><img src=\"images/close.png\" style=\" float: right; \"  onClick=\"hideIt('action_edit_".$action->getProperty('id')."')\" title=\"Закрыть форму\" />".($action->getFormManager()->generateActionEditForm(array('action' => $action)))."</div>";
		}
		if (($user_permissions[getParentModule()][getParentChildModule()]['can_admin']) and ($action->isInteractive()) and ($action->getProperty('status_id') == Constants::ACTION_STATUS_COMPLETED) and ($action->getProperty('type_id') <> Constants::ACTION_TYPE_INFO) and ($process->getProperty('status_id') == Constants::PROCESS_STATUS_IN_PROGRESS)) {
			print "&nbsp;<img class=\"action\" src=\"images/reset.png\" onClick=\"confirmIt('?module=".getParentModule().DIRECTORY_SEPARATOR.getChildModule().DIRECTORY_SEPARATOR."list&action=undo2action&action_id=".$action->getProperty('action_id')."&action_instance_id=".$action->getProperty('id')."&project_id=".PROJECT_ID."&process_id=".PROCESS_ID."&project_instance_id=".PROJECT_INSTANCE_ID."&process_instance_id=".PROCESS_INSTANCE_ID."', '_top', true);\" title=\"Откатиться на это действие\" />";
		}
		print "</td>";
		print "</tr>\n";
	}
