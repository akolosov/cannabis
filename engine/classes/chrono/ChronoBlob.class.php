	...
	class ChronoBlob extends Core {
		...
		function __construct($owner = NULL, $id = 0, $full = false) {
			if (($id > 0) && ($owner <> NULL)) {
				...
				$blob = $this->getConnection()->execute('select id, blob_id, blob_value_id, value_id'.($full?', blob':'').' from cs_chrono_blob where id = '.$this->_id)->fetch();
			...
			$this->_blob['[model]'] = "CsChronoBlob";
		}

		static function getBlob($owner = NULL, $value_id = 0, $full = false) {
			if (($value_id > 0) && ($owner <> NULL)) {
				$blob = $owner->getConnection()->execute('select id from cs_chrono_blob where value_id = '.$value_id)->fetch();
				
				$result = new ChronoBlob($owner, $blob['id'], $full);
				$result->_owner = $owner;
