...
class TransportProtocolJabber extends TransportProtocol {
    var $connection = null;
    var $session = array();
    var $resource = 'TransportProtocolJabber.class.php';
    var $log = array();
    ...
    function __construct($owner = NULL, $options = array())
    {
   		parent::__construct($owner, 0, $owner->getConnection());

   		$options = array_merge(array('login' => NULL, 'password' => NULL, 'security' => SECURITY_NONE, 'port' => 5222, 'host' => ''), $options);
        ...
        if (!Jabber::check_jid($options['login'])) {
            $this->log('Error: Jabber ID is not valid: ' . $options['login']);
            return false;
        }

        // Extract data from user@server.org
        list($username, $server) = explode('@', $options['login']);

        // Decide whether or not to use encryption
        if ($options['security'] == SECURITY_SSL && !Jabber::can_use_ssl() || $options['security'] == SECURITY_TLS && !Jabber::can_use_tls()) {
            $options['security'] = SECURITY_NONE;
        }

        $this->session['security'] = $options['security'];
        $this->server              = $server;
        $this->user                = $username;
        $this->password            = $options['password'];

        if ($this->open_socket( ($options['host'] != '') ? $options['host'] : $options['server'], $options['port'], $options['security'] == SECURITY_SSL)) {
            $this->send("<?xml version='1.0' encoding=DEFAULT_CHARSET ?" . ">\n");
	...
	function xmlize($data, $WHITE=1, $encoding=DEFAULT_CHARSET) {
