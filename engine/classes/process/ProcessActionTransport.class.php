...
class ProcessActionTransport extends Core {
	...
	function __construct($owner = NULL, $id = 0, $options = array('data' => NULL)) {
		if (($id > 0) && ($owner <> NULL)) {
			...
			if (is_null($options['data'])) {
				$transport = $this->getConnection()->execute('select * from process_action_transports_list where id = '.$this->_id)->fetch();
			} else {
			...
			$this->_transport['[model]']		= "CsProcessActionTransport";
		}
	}
	...
	static function import(array $transportdata = array(), $connection = NULL) {
		...
		$transport = self::importData("ProcessActionTransport", "CsProcessActionTransport", $transportdata, (is_resource($connection)?$connection:$engine->getConnection()));
		...
		if (($find = $transport->getConnection()->execute('select * from '.$transport->getConnection()->getTable($transport->getProperty('[model]'))->getTableName().' where action_id = '.$transport->getProperty('action_id').' and transport_id = '.$transport->getProperty('transport_id'))->fetch()) != false) {
