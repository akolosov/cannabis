...
$action = isset($_GET['action']) ? $_GET['action'] : null;
	...
	// проверяю есть ли печеньки 
	if (!isset($_COOKIE['user_name']) || !isset($_COOKIE['user_passwd'])) {
		// ежели нет печенья - отправляю за ним на канапляное поле 
		$URL = ($_SERVER['HTTPS']?"https://":"http://").$_SERVER['SERVER_NAME']."/?module=common/authorize&backto=".urlencode("/forum/login.php?action=in");
		header ("Location: $URL");
	}
	
	// читаю печеньки 
	$cann_username = trim($_COOKIE['user_name']);
	$cann_password = trim($_COOKIE['user_passwd']);
	$cann_usermail = trim($_COOKIE['user_mail']);
		
	// проверяю есть ли пользователь с таким мылом 
	$check_result = $db->query('SELECT username FROM '.$db->prefix.'users WHERE email=\''.$cann_usermail.'\'') or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	if ($db->num_rows($check_result)) {
		// если есть - ниха не делаю 
		$check_result = array();
	} else {
		// если нет - задаю ему параметры 
		$now = time();
		$intial_group_id = ($pun_config['o_regs_verify'] == '0') ? $pun_config['o_default_user_group'] : PUN_UNVERIFIED;
		$password_hash = $cann_password /*pun_hash($password1)*/;
	...
	// добавляю пользователя 
		...
		$db->query('INSERT INTO '.$db->prefix.'users (username, group_id, password, email, email_setting, save_pass, timezone, language, style, registered, registration_ip, last_visit) VALUES(\''.$db->escape($cann_username).'\', '.$intial_group_id.', \''.$password_hash.'\', \''.$cann_usermail.'\', '.$email_setting.', '.$save_pass.', '.$timezone.' , \''.$db->escape($language).'\', \''.$pun_config['o_default_style'].'\', '.$now.', \''.get_remote_address().'\', '.$now.')') or error('Unable to create user', __FILE__, __LINE__, $db->error());
...
if ($action == 'in')
{
	...
	$form_password = trim($_COOKIE['user_passwd']);
//	$form_password = trim($_POST['req_password']);
	...
	$form_username = $cann_username;
	$form_password = $cann_password;
	...
	if (!empty($db_password_hash))
	{
		...
		$form_password_hash = $form_password; /*pun_hash($form_password);*/	// This could result in either an SHA-1 or an MD5 hash (depends on $sha1_available)
	...
	if (!$authorized)
		message($lang_login['Wrong user/pass'].' <a href="login.php?action=forget">'.$lang_login['Forgotten pass'].' ('.$form_password_hash.') ('.$db_password_hash.')</a> ');
...
//	pun_setcookie(1, random_pass(8), time() + 31536000);
	pun_setcookie(1, random_pass(8), 0);
...
<div class="blockform">
	...
	<div class="box">
	<!-- <?php print "<span style='color:red;'>ОТЛАДКА НАХ: "; print_r($_COOKIE); print "</span>"; ?> -->
				...
				<fieldset>
					<legend><?php echo $lang_login['Login legend'] ?></legend>
						<div class="infldset">
							...
							<label class="conl"><strong><?php echo $lang_common['Username'] ?></strong><br /><input type="text" name="req_username" size="25" maxlength="25" tabindex="1" value="<?php echo $_COOKIE['user_name']; ?>" /><br /></label>
							<label class="conl"><strong><?php echo $lang_common['Password'] ?></strong><br /><input type="password" name="req_password" size="16" maxlength="16" tabindex="2" value="<?php echo $_COOKIE['user_passwd']; ?>" /><br /></label>
