...
if (isset($_POST['form_sent']))
{
	...
	if ($hide_smilies != '1') $hide_smilies = '0';
		...
		$del_file = (isset($_POST['delfile'])) ? intval($_POST['delfile']) : 0;
		if ($del_file <> '1') $del_file = '0';	
	
		if ($_FILES["filename"]["size"] > 2) {
			// Взвешиваю файл (вложение)
			$file_error = "0";
			if ($_FILES["filename"]["size"] > 1024*20*1024) {
				echo "Оченя бальшой файла, начальника...";
				$file_error = "1";
			}
		...
		if ($change_file == '1') { $editfile = 'change'; }
		elseif ($del_file == '1') { $editfile = 'del'; }
		else { $editfile = 'no'; }
	...
	if (empty($errors) && !isset($_POST['preview']))
	{
			...
			// Кладу файл на сервер
			if (($file_error == "0") && ($change_file == "1") && (is_uploaded_file($_FILES["filename"]["tmp_name"]))) {
				move_uploaded_file($_FILES["filename"]["tmp_name"], $_SERVER['DOCUMENT_ROOT']."/forum/attach/".$_FILES["filename"]["name"]);
			}
			// Либо выковыриваю файл с сервера
			if ($editfile == 'del') {
				require 'config.php';
				$resultat = pg_query($dbconnect, 'SELECT file, poster FROM '.$db->prefix.'posts WHERE id='.$id);
				while ($rowl = pg_fetch_row($resultat)) {
  					$attacha = $rowl[0];
				}
				unlink($_SERVER['DOCUMENT_ROOT'].$attacha);
			}
		...
		redirect('viewtopic.php?pid='.$id.'#p'.$id, $lang_post['Edit redirect'], $db->prefix, $_FILES["filename"]["name"], $id, $editfile);
	}
}
...
<?php endif; ?>						<label><?php echo $lang_common['Message'] ?><br />
						...
						<br><input type="checkbox" name="delfile" value="1" /> Удалить прикрепленный файл<br>
						<br>Добавить/Заменить прикрепленный файл: <input type="file" name="filename">
					</div>
				</fieldset>
<?php
