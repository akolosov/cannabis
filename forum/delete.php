...
if (isset($_POST['delete']))
{
	...
	if ($is_topic_post)
	{
		...
		update_forum($cur_post['fid']);
			...
			// Удаляю файлы с сервера
			$resultat = pg_query($dbconnect, 'SELECT file FROM '.$db->prefix.'posts WHERE topic_id='.$cur_post['tid']);
			$files = pg_fetch_array($resultat, 0, PGSQL_NUM);
				foreach ($files as $file) {
  					unlink($_SERVER['DOCUMENT_ROOT'].$file[0]);
  					echo 'Файл '.$_SERVER['DOCUMENT_ROOT'].$file[0].' удален';
				}			
