...
<?php if (count($user_contacts)) echo "\t\t\t\t\t".'<dd class="usercontacts">'.implode('&nbsp;&nbsp;', $user_contacts).'</dd>'."\n"; ?>
			...
			<div class="postright">
				...
				<div class="postmsg">
					...
					// Проверяю есть ли вложения /////
					require 'config.php';
					$resultat = pg_query($dbconnect, 'SELECT file, poster FROM '.$db->prefix.'posts WHERE id='.$cur_post['id']);
					while ($rowl = pg_fetch_row($resultat)) {
  						$attacha = $rowl[0];
					}
					$attachl = str_replace("/forum/attach/", "", $attacha);
					$attach_link = "<br><p>Вложение: <a href='".$attacha."'>".$attachl."</a></p>";
					// Ежели есть вложение - показываю
					if ($attach_link <> "<br><p>Вложение: <a href=''></a></p>" && $attach_link <> "") {
						echo $attach_link."\n";
					}
					// Конец вложений /////
