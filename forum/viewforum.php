...
if ($db->num_rows($result))
{
	while ($cur_topic = $db->fetch_assoc($result))
	{
		...
		if ($cur_topic['moved_to'] == null)
			$last_post = '<a href="viewtopic.php?pid='.$cur_topic['last_post_id'].'#p'.$cur_topic['last_post_id'].'">'.format_time($cur_topic['last_post']).'</a><br><span class="byuser"><span style="color:#666;">'.$lang_common['by'].'</span>&nbsp;'.pun_htmlspecialchars($cur_topic['last_poster']).'</span>';
		else
			$last_post = '&nbsp;';
		...
		if ($cur_topic['moved_to'] != 0)
			$subject = $lang_forum['Moved'].': <a href="viewtopic.php?id='.$cur_topic['moved_to'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a><br><span class="byuser"><span style="color:#666;">'.$lang_common['by'].'</span>&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else if ($cur_topic['closed'] == '0')
			$subject = '<a href="viewtopic.php?id='.$cur_topic['id'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a><br><span class="byuser"><span style="color:#666;">'.$lang_common['by'].'</span>&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
		else
		{
			$subject = '<a href="viewtopic.php?id='.$cur_topic['id'].'">'.pun_htmlspecialchars($cur_topic['subject']).'</a><br><span class="byuser"><span style="color:#666;">'.$lang_common['by'].'</span>&nbsp;'.pun_htmlspecialchars($cur_topic['poster']).'</span>';
